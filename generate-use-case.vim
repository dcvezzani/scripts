...
function! GenerateUseCase() range
  let $WRAP_MIN = "13"
  ...
  execute 'r! ~/scripts/cat-to-file.sh '.shellescape(@w, 1).' ~/scripts/generate-use-case.js >/dev/null 2>&1'
endfunction

function! GenerateUseCaseCode() range
  let $WRAP_MIN = "13"

  let @w = GetVisualSelection(visualmode())
  let @z = 'gvdk' | normal @z

  execute 'r! GENERATE_CODE=true ~/scripts/cat-to-file.sh '.shellescape(@w, 1).' ~/scripts/generate-use-case.js'
endfunction
